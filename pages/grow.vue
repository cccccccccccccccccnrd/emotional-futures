<template>
  ...
  <div v-if="step === 6" class="h-[70%] flex flex-col">
    ...
    >
      <div>
        <p>{{ selectedEmotion.name }} {{ isRevealed ? selectedRelationshape.name : '' }}</p>
        <p class="mt-4">
          Activation partner <strong>{{ selectedFriend.name }}</strong>
        </p>
      </div>
      <div>
        ...
        <div v-else>
          <p>{{ selectedRelationshape.id ? selectedEmotion.prompts[selectedRelationshape.id - 1] : '' }}</p>
        </div>
      </div>
      <div>
        <div v-if="!isRevealed">
  ...
  <div v-if="step === 7" class="h-[70%] flex flex-col">
    <p>The {{ selectedRelationshape.name }} Relationshape requires {{ selectedRelationshape.accounting.length }} Accounting step{{ selectedRelationshape.accounting.length > 1 ? 's' : '' }}:</p>
    <div>
      <div v-for="account in selectedRelationshape.accounting" class="mt-4">
        <div @click="selectedAccount = account; step = 8" class="p-3 border">{{ account.name }} {{ completedAccounts.find((a: any) => a.name === account.name) ? 'X' : '' }}</div>
      </div>
      {{ completedAccounts }}
    </div>
  </div>
  ...
  <div v-if="step === 11" class="h-[70%] flex flex-col">
    <p>Final Balance will become available as soon as your Accounterpart confirms their investment.</p>
  </div>
  <div
    v-if="step === 11"
    class="h-[20%] flex flex-col justify-center items-center"
...
function setActivation(a: any) {
  ...
  selectedFriend.value = friends.find(
    (f: any) => f.user_id === a.friend_id || a.user_id === f.user_id
  )
